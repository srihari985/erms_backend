...
public interface TokenRepository extends JpaRepository<Token, Long> {
    ...
    @Query("SELECT t FROM Token t WHERE (t.organization.id = :userId OR t.admin.id = :userId OR t.managers.id = :userId OR " +
            "t.saleManager.id = :userId OR t.operationTechnicalLeadEntity.id = :userId OR t.technician.id = :userId OR t.sales.id = :userId) " +
