...
//    }
    ...
    @PostMapping("/register/operationTechLead/{salesManagerId}")
    public ResponseEntity<AuthenticationResponse> registerOperationTechLeadManager(
            @PathVariable String salesManagerId,
            @RequestParam String firstname,@RequestParam String lastname, @RequestParam String email,
            @RequestParam Role role) {
        try {
            // Call the service to handle technician registration
            AuthenticationResponse response = authenticationService.registerOperationTechLead(salesManagerId,firstname, lastname, email, role);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new AuthenticationResponse(e.getMessage()));
        }
